...
def evaluate_recall_reranked():
    ...
    for idx, row in enumerate(answerable_rows, 1):
        ...
        try:
            ...
            if is_hit:
                hits += 1
                print(f"  -> RERANKED RECALL@4: HIT (Files: {files})")
            else:
                print(f"  -> RERANKED RECALL@4: MISS (Expected: '{answer_file}', Got: {files})")
        except Exception as e:
            print(f"  -> RERANKED RECALL@4: ERROR ({e})")
    score = hits / len(answerable_rows)
    print(f"Reranked Retrieval Recall@4             : {score:.4f}")

if __name__ == "__main__":
    evaluate_recall_reranked()
